...
{
    public class MPackTransferProtocol : ITransferProtocol<MPack>
    {
        public IObservable<MessageRecievedEventArgs<MPack>> MessageRecieved => _observable.AsObservable();
        ...
        private Subject<MessageRecievedEventArgs<MPack>> _observable;

        public MPackTransferProtocol()
        {
            _observable = new Subject<MessageRecievedEventArgs<MPack>>();
        }
        ...
        public async Task WriteAsync(MPack message)
        {
            var packet = message.EncodeToBytes();
        ...
        public void Write(MPack message)
        {
            var packet = message.EncodeToBytes();
            byte[] lengthPrefix = BitConverter.GetBytes(packet.Length);
        ...
        private void EndRead(IAsyncResult result)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    var data = MPack.ParseFromBytes(_dataBuffer);
                    _observable.OnNext(new MessageRecievedEventArgs<MPack>(data));
